  ...
  methods: {
    ...
    */
    ...
    touchEnd(e) {
      ...
      // 用户拖动的距离大于0，也就是手指往右滑动, 并且移动的宽度大于，一个页面的宽度乘以0.25的比例，也就是移动了四分之一就可以进入到下一个页面了
      else if(this.distance > 0 && currentMove > (this.totalWidth * this.moveRatio)) {
        this.currentIndex--
      }

      // 用户拖动的距离小于0，也就是手指往左滑动, 并且移动的宽度大于，一个页面的宽度乘以0.25的比例，也就是移动了四分之一就可以进入到下一个页面了
      else if(this.distance < 0 && (currentMove > (this.totalWidth * this.moveRatio))){
        this.currentIndex++
      }
